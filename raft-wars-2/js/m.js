...
var MainGame = {
    ...
    addButton: function(link, vLayer, vX, vY, onClick, vText, vW, vH, vSize) {
        ...
        if (vText != "")
            btn.text = MainGame.addText(800, vX, vY, vText, vLayer, vSize, "#FFFFFF", .5, .5);
        return btn
    },
    ...
    loadSaves: function() {
        ...
        if (typeViking != undefined && typeViking != "null")
            MainGame.typeViking = Number(typeViking);
        ...
        if (countBalls3 != undefined && countBalls3 != "null")
            MainGame.countBalls3 = Number(countBalls3);
...
Bot.prototype.setSuperRocket = function() {
    if (this.isSkinWithRocket)
        return;
    if (this.skin == "sman")
        this.obj.setSkinByName("sman4");
    if (this.skin == "simon")
        this.obj.setSkinByName("simon4");
    if (this.skin == "dkid")
        this.obj.setSkinByName("dkid4");
    if (this.skin == "sman2")
        this.obj.setSkinByName("sman5");
    if (this.skin == "simon2")
        this.obj.setSkinByName("simon5");
    if (this.skin == "dkid2")
        this.obj.setSkinByName("dkid5");
    if (this.skin == "sman3")
        this.obj.setSkinByName("sman6");
    if (this.skin == "simon3")
        this.obj.setSkinByName("simon6");
    if (this.skin == "dkid3")
        this.obj.setSkinByName("dkid6");
    this.obj.setAnimationByName(0, "gogun", false);
    this.obj.setToSetupPose();
...
MainGame.Game.prototype = {
        ...
        if (nameAnim == "idle") {
            ...
            if (MainGame.typeViking == 0) {
                ...
                if (r == 7 || r == 8)
                    r = 9;
                if (r == 3)
                    r = MyMath.getRandomInt(4, 6)
            }
            if (r == 6 && this.stat_shots == 0)
                r = 2;
    ...
    nextTurn: function() {
        ...
        if (this.hero1.hpNow > 0 || this.hero2.hpNow > 0)
            isLiveHeroes = true;
        if (MainGame.typeBoat == 4 && (isLiveHeroes || this.hero3.hpNow > 0))
            isLiveHeroes = true;
        if (isLiveHeroes) {
            if (this.turnNum == 1) {
                ...
                if (this.striker.hpNow <= 0) {
                    ...
                    if (MainGame.typeBoat == 4 && this.hero3.hpNow > 0)
    ...
    shieldEvent: function(vBool) {
        ...
        if (!vBool)
            numAnim = 8;
        if (this.hero1 && this.hero1.hpNow > 0)
            this.hero1.updateAnim(numAnim);
        if (this.hero2 && this.hero1.hpNow > 0)
            this.hero2.updateAnim(numAnim);
        if (MainGame.typeBoat == 4) {
            if (this.hero3 && this.hero1.hpNow > 0)
                this.hero3.updateAnim(numAnim)
        }
    },
    levelWin: function() {
        this.isLevelCompleted = true;
    ...
    attackEnemy: function(vNumStiker) {
        ...
        if (this.hero2.isLive)
            arHeroes.push(this.hero2);
        if ((MainGame.typeBoat == 2 || MainGame.typeBoat == 3) && this.dog.isLive)
            arHeroes.push(this.dog);
        if (MainGame.typeBoat == 4 && this.hero3.isLive)
            arHeroes.push(this.hero3);
        ...
        if (MainGame.typeBoat == 2 && (arHeroes[r].skin == "bear" || arHeroes[r].skin == "greybeard"))
            goalY += 25;
        this.stepAttack++;
        var awp = 14 - MainGame.levelNum - this.stepAttack;
        if (MainGame.levelNum == 4)
            awp = Math.round(awp * .8);
    ...
    setBallSensor: function() {
        ...
        if (this.ball1.visible)
            this.ball1.body.sensor = false;
        if (this.ball2.visible)
            this.ball2.body.sensor = false;
        if (this.ball3.visible)
            this.ball3.body.sensor = false
    },
    ...
    update: function() {
        if (MainGame.showFPS)
            MainGame.textFPS.setText("FPS: " + game.time.fps);
...
MainGame.Comics.prototype = {
    ...
    initComics: function() {
        ...
        if (MainGame.comicsNum >= 5)
            this.spritesheet = "ss_comics2";
        if (MainGame.comicsNum != 9)
            MainGame.fadeOut();
        this.stepAction = 0;
        if (game.width > 1300) {
        ...
        case 2:
            if (this.effectBulk1)
                this.effectBulk1.visible = false;
            if (this.effectBulk2)
                this.effectBulk2.visible = false;
            this.whiteBack = MainGame.addFill(this.layerComics, 16777215, 1, game.width, 600, -this.halfWG, 0);
    ...
    checkTimer: function(vP, vNum) {
        ...
        if (vNum == 0 && this.stepAction == 5)
            isNextStep = true;
        if (vNum == 1 && this.stepAction == 7)
            isNextStep = true;
        if (vNum == 2 && this.stepAction == 8)
            isNextStep = true;
        if (vNum == 3 && this.stepAction == 0)
            isNextStep = true;
        if (vNum == 4 && this.stepAction == 2)
            isNextStep = true;
        if (vNum == 5 && this.stepAction == 6)
            isNextStep = true;
        if (vNum == 6 && this.stepAction == 0)
            isNextStep = true;
        if (vNum == 7 && this.stepAction == 3)
            isNextStep = true;
        if (vNum == 8 && this.stepAction == 0)
            isNextStep = true;
        if (vNum == 9 && this.stepAction == 2)
            isNextStep = true;
        if (isNextStep) {
            this.stepAction++;
            this.checkStep()
